...
class Settings extends Component {
  static navigationOptions = ({ navigation }) => {
    ...
    return {
      title: 'Settings',
      headerLeft: isReady
        ? <Button title='Cancel' onPress={state.params.cancel} />
        : undefined,
      headerRight: isReady
        ? <Button title='Done' onPress={state.params.updateUser} />
        : undefined,
      tabBarLabel: 'Profile',
      tabBarIcon: ({ tintColor }) => (
        <EvilIcons color={tintColor} name='user' size={30} />
      )
    }
  };
  ...
  updateUser () {
    ...
    this.props.updateUser({ username, avatar }).then(({
      data: { updateUser }
    }) => {
  ...
  refreshNavigation (props, state) {
    ...
    navigation.setParams({
      mode: (state.username && user.username !== state.username) || state.avatar
        ? 'ready'
        : undefined,
